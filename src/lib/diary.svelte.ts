...
export interface MediaRecord extends MediaMeta {
  blob: Blob; // possibly encrypted (application/x-gpe-enc;orig=...)
}

const photos = browser ? localforage.createInstance({ name: 'gpe', storeName: 'placephotos' }) : null;
const audios = browser ? localforage.createInstance({ name: 'gpe', storeName: 'placeaudios' }) : null;
...
export async function getPhotoURL(id: string): Promise<string | null> {
  ...
  if (!rec) return null;
  let blob = rec.blob;
  if (blob.type.includes('x-gpe-enc')) {
