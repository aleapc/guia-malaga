...
export const doneStore = $state<{ ids: string[] }>({ ids: load<string[]>('gpe-done', []) });
export const favStore = $state<{ ids: string[] }>({ ids: load<string[]>('gpe-fav', []) });
export const notesStore = $state<{ map: Record<string, string> }>({
  map: load<Record<string, string>>('gpe-notes', {})
});
