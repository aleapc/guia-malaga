...
export function encodeSync(): string {
  ...
  return 'GP1.' + b64uEncode(JSON.stringify(payload));
}

export function decodeSync(code: string): SyncData | null {
  try {
    const m = code.trim().match(/GP1\.([A-Za-z0-9_-]+)/);
    if (!m) return null;
...
export function shareUrl(code: string): string {
  return `https://aleapc.github.io/guia-malaga/sync#s=${code}`;
}

export function whatsappUrl(code: string): string {
  const text =
    `🇪🇸 *Guia Málaga* — meus lugares marcados!\n` +
