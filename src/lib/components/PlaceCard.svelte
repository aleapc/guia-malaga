<script lang="ts">
  ...
  import { categoryById, cityById, type Attraction } from '$lib/content';
  import { isDone, isFav, toggleFav } from '$lib/state.svelte';
  import { isoWeekday, todayISO } from '$lib/dates';
  import Photo from './Photo.svelte';
  import Badge from './Badge.svelte';

  let { a, showCity = false }: { a: Attraction; showCity?: boolean } = $props();

  const cat = categoryById(a.categoryId)!;
  const cityInfo = $derived(showCity ? cityById(a.city) : undefined);
...
<a href="{base}/local/{a.id}" class="flex gap-3 rounded-2xl bg-white p-2.5 shadow-sm active:scale-[0.99]">
  ...
  <div class="min-w-0 flex-1">
    ...
    <div class="mt-1.5 flex flex-wrap gap-1.5">
      {#if cityInfo}<Badge tone="city">{cityInfo.emoji} {cityInfo.label}</Badge>{/if}
