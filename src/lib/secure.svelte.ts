...
const SALT_KEY = 'gpe-sec-salt';
const VERIFY_KEY = 'gpe-sec-verify';
...
export async function encryptBlob(blob: Blob): Promise<Blob> {
  ...
  return new Blob([out], { type: `application/x-gpe-enc;orig=${blob.type}` });
}

export async function decryptBlob(blob: Blob): Promise<Blob> {
  if (!blob.type.includes('x-gpe-enc')) return blob; // plaintext
  if (!key) throw new Error('locked');
...
export async function seedEncrypt(plain: string, password: string): Promise<string> {
  ...
  return `gpeseed:${bufToB64(salt.buffer)}:${bufToB64(iv.buffer)}:${bufToB64(ct)}`;
}
...
export async function setPin(pin: string): Promise<void> {
  ...
  localStorage.setItem(VERIFY_KEY, await encryptStr('gpe-ok'));
  lock.enabled = true;
  lock.locked = false;
}
...
export async function unlock(pin: string): Promise<boolean> {
  ...
  try {
    if ((await decryptStr(ver)) === 'gpe-ok') {
      lock.locked = false;
      return true;
    }
  } catch {
...
export function resetProtected() {
  ...
  localStorage.removeItem('gpe-trip');
  localStorage.removeItem('gpe-attach-meta');
  // attachment + couple-photo blobs are cleared by their stores on next write; drop the DB:
  indexedDB?.deleteDatabase?.('gpe');
  key = null;
  lock.enabled = false;
  lock.locked = false;
}
